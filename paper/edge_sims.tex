...
Conservation triage, like all sound decision-making, requires
\replaced{a method}{an index with which}
% R2 was concerned you said triage required an index (as was the
% editor); they're wrong, but reference this here
to quantify the relative urgency or importance for conservation among
a set of options. This allows scientists and policy-makers to use data
to quantify need and inform conservation decision-making and
management activities. One triage strategy uses the EDGE metric to
identify and prioritize species that are Evolutionarily Distinct and
Globally Endangered \autocite{Isaac2007}.  Evolutionary
Distinctiveness (ED) measures the relative contributions made by each
species within a particular clade to phylogenetic diversity, assigning
each branch length equally to all the subtending species
\autocite{Redding2003, Isaac2007}. Global Endangerment (GE), assigns
numerical values to each of the International Union for Conservation
of Nature (IUCN) Red List Categories. As species become increasingly
threatened and are placed into categories of increasing concern
(\emph{e.g.} from Vulnerable to Endangered), the GE numerical value
increases. A species’ EDGE score is an aggregate value intended to
equally reflect a species’ evolutionary distinctiveness and
conservation status \autocite[even if it does not always in practice;
see][]{Pearse2015}.
...
from missing data. \added{Addressing, and hopefully improving, our ability to
...
Here we attempt to quantify the effect of one sort of phylogenetic uncertainty---the effect of missing species on EDGE rankings---and assess the degree to
...
clade and total phylogeny sizes 100 times \added{when using a pure-birth Yule
...
\added{We present, in the supplementary materials, two additional sets
  of analyses intended to examine the impact that past extinction
  rates may have played on our analyses. These simulations incorporate
  conditions of past extinction at low and high rates using
  'gieger::sim.bdtree' \autocite[setting parameters for low extinction
  at \texttt{b=1} and \texttt{d=0.5} and high extinction \texttt{b=1}
  and \texttt{d=0.95};][]{Pennell2014}. These models represent large
  departures from our main simulations (which have \texttt{b=1} and
  \texttt{=0}), and so we performed only 5 replicates per set of
  parameter combinations as our only aim was to detect any major
  differences in our results stemming from these changes. Otherwise,
  these simulations were identical to those present in the main text.}
...
\added{As we show in the supplementary materials, the simulation (and
  subsequent imputation) of phylogenies under models incorporating
  extinction rates (\emph{i.e.}, not Yule models) had qualitatively
  identical results. We do not, therefore, discuss them in detail here.}
...
\added{In this study, we have examined imputation under three separate
  models of diversification: pure-birth Yule models (presented in the
  main text), and models with relatively high and low rates of
  extinction (both in the supplementary materials).} We acknowledge
that lineages evolve in more complex ways\added{, although we suggest
  that focusing on these fundamental models of diversification makes
  our results more broadly applicable. We suggest that a method should
  perform well under basic conditions, and as such these results form
  an appropriate benchmark, particularly given we can see no reason to
  suppose that more complex models should increase model performance.}
\deleted{ Our results are derived solely from simulations under a
  simple model of diversification---the Yule model. We acknowledge
  that lineages evolve in more complex ways. However, such
  complexities are unlikely to make imputation easier, and they would
  have to be modelled and quantified in empirical studies. We suggest
  that focusing on the simplest model, of diversification makes our
  results more generalizable.}  Further, we focus here solely on the
results from a single imputation in each simulation, despite,
empirically, biologists reporting average ED scores calculated across
pseudo-posterior distributions of many imputed phylogenies
\autocite{Kuhn2011}. Thus our results show that the variation within
these pseudo-posterior distributions is likely very large. It is
well-known that such imputation methods are not biased
\autocite[indeed, this was originally shown by][]{Kuhn2011}: here we
emphasize that the uncertainty they introduce is sufficiently large
such that they may be less informative than previously has been
thought.

% Bodie: I've cut this because, to be honest with you, I don't think
% we need to write this. It was a stupid reviewer comment, and I think
% it weakens our paper to put this in here. I think it's pretty
% obvious to anyone with half a brain why empirical data wouldn't help
% here: whether EDGE imputation works in birds is not our question
% here, our question is whether it works full stop. We're also sort of
% lying in it: we didn't look at empirical data to spare people's
% embarrassment, because we know it doesn't work. Let's just avoid the
% issue, I think.
\iffalse
\added{We acknowledge that empirical data would indeed push our argument further
and certainly be useful. Using empirical data to verify the patterns that we see
in our results would only strengthen our conclusions. Even still, we anticipate
that our results will improve upon the understanding of the robustness of ED
estimates and how phylogenetic uncertainty affects EDGE-type metrics.
Additionally, we hope that our results will lead to further discussion and
empirical testing of our conclusions.} 
\fi
...
\added{We emphasize that we have not demonstrated, and do not argue,
  that missing species cannot affect ED scores.}
We simply demonstrate
...
scores of species for which we have data\added{, and that (in our opinion)
  ED scores are remarkably robust to missing species.
  Other patterns and scenarios for species to be missing could
  easily lead to systematic biases of ED scores, and so very effort
  should be made to gather accurate phylogenetic information for all
  species within a clade before prioritisation is carried out.}
...
status. \added{With this in mind, we hope to provide clear guidelines, along
...
to account for the relatively minor impact of missing species, we suggest that
conservation biologists should, without accounting for phylogenetic uncertainty,
focus on the species for which they have data. While we have not explored this
...
EDGE scores and rankings \autocite{Pearse2015}. If biologists are concerned
about the impact of missing species on known species' ED(GE) scores we see no
harm in being precautionary and using imputation. It is important, however, to
focus on known sources of potential error, and so we would encourage biologists
to incorporate uncertainty in species with phylogenetic data as a priority.
% Something about known unknowns an unknown unknowns in here?
...
\added{In light of the suggestions and caveats that we have outlined, we
acknowledge that each particular user of EDGE-type metrics will need to consider
how uncertainty and how using imputation affects their end goal. For instance,
our suggestion of focusing on species for which they have do data, without
accounting for phylogenetic uncertainty, could help conservationists move
swiftly to set priorities and act now. However, this strategy is not to say that
we should always neglect species for which we do not have data. Those looking to
improve upon EDGE-type metrics might want to find better strategies of
imputing species. However, these same users should consider our findings when
developing methods to maintain accuracy and precision for species which they do
have data.}
