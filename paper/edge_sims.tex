...
\replaced{In our first set of simulations, all}{All} trees (both starting and
...
\added{Therefore, we performed two additional sets of simulations under more
elaborate models of evolution to test the performance of imputation across model
complexity. These additional simulations incorporate conditions of past
extinction at low and high rates using 'gieger::sim.bdtree' \autocite[setting
parameters for low extinction at \texttt{b=1} and \texttt{d=0.5} and high
extinction \texttt{b=1} and \texttt{d=0.95};][]{Pennell2014}. These two
additional sets of simulations were done with less repititions than the first
set. This was done in order to verify that our results were consistent across
models of evolution while not overly expending computational resources.} We used
...
\replaced{We}{, and we} performed \replaced{100 replicates}{100 replicate
simulations} of each parameter combination \added{in our first set of
simulations and 5 replicates in our two additional set of simulations}. All our
...
\added{We have provided results of simulations using three separate models of
evolution varying in complexity from one another.} \replaced{In this discussion
we will focus on our}{Our} results \deleted{are} derived solely from simulations
under a simple model of diversification---the Yule model. We acknowledge that
lineages evolve in more complex ways. However, \added{we have shown that} such
complexities \replaced{do not}{are unlikely to} make imputation easier
\replaced{(see Appendix 4 \& 5 in Supporting Information)}{, and they would have
to be modelled and quantified in empirical studies}. \replaced{Given that our
results are similar across simulations, we}{We} suggest that focusing on
\added{the results from a single model,} the simplest model, of diversification
makes our \replaced{discussion easier to follow while not missing any important
details}{results more generalizable}. Further, we focus here solely on the
...
\added{We acknowledge that species could be missing from a phylogeny in ways
which we do not fully understand.} \replaced{However, we}{We} simply demonstrate
