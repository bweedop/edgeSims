...
harm in being precautionary and using imputation. \added{We say this given that
we cannot say that our suggestion accounts universally to some unknown
phylogenetic patterns.} It is important, however, to focus on known sources of
potential error, and so we would encourage biologists to incorporate uncertainty
in species with phylogenetic data as a priority.
...
acknowledge that each particular user of any method of triage in conservation
will need to weigh the benefits and drawbacks. A clear benefit to users is a
researched method to set priorities informed by multiple sources of crucial
data. While resources within the scope of conservation are limited and possibly
insufficient, triage methods can assist users in directing those limited
resources in a more effective manner. With a range of triage methods at hand,
priorities can be set to direct efforts and resources to species which are at
risk of extinction and represent a sizeable segment of evolutionary history,
functional trait diversity or how likely a species is to succeed given the
effort. However, there are obvious drawbacks as well. Triage methods take time
to understand, develop and research. If aspects of conservation triage methods
are not well understood, priorities could be disoriented and set such that
resources are misdirected. However, our study addresses one such aspect within
EDGE-type methods. With our results, we offer an explanation of how uncertainty
and using imputation might affect triaging methods such as EDGE. Our conclusions
and their implications should be considered carefully by each particular user of
EDGE and other, similar triage methods.}
